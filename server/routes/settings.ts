...
settingsRouter.get('/connectors', async (_req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to load connectors', detail: (err as Error).message })
  }
})
...
settingsRouter.put('/connectors/:id', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to save connector', detail: (err as Error).message })
  }
})
...
settingsRouter.post('/connectors/:id/test', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to test connector', detail: (err as Error).message })
  }
})
