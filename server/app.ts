...
export interface CreateAppOptions {
  /** Dashboard token gating /api. Defaults to DASHBOARD_TOKEN env; empty = open (dev/test). */
  dashboardToken?: string
}

/**
 * Builds the Express app with all middleware and routers wired, but does NOT
 * call listen() or start any background jobs (memory collector, briefing
 * scheduler, Discord bot). index.ts owns the process lifecycle; tests import
 * this factory to exercise routes in-process via supertest.
 */
export function createApp(options: CreateAppOptions = {}): express.Express {
  const app = express()
  const dashboardAuth = new DashboardAuth(options.dashboardToken ?? process.env.DASHBOARD_TOKEN ?? '')
